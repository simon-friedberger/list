...
func (c *GHRepo) apiClient() *github.Client {
	if c.client == nil {
...
func (c *GHRepo) PSLForPullRequest(ctx context.Context, prNum int) (withoutPR, withPR []byte, err error) {
...
// appropriate backoff.
func (c *GHRepo) getPRCommitInfo(ctx context.Context, prNum int) (withoutPRCommit, withPRCommit string, err error) {
...
func (c *GHRepo) PSLForHash(ctx context.Context, hash string) ([]byte, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
